...
/**
 * ICalendarAdapter defines an interface for retrieving calendar event data. All new calendar types
 * must define an adapter using this interface, then be registered in the spring context files.
 *
 * @author Jen Bourey
 */
public interface ICalendarAdapter {
  ...
  /**
   * Get events for the defined calendar and time period. The user's PortletRequest is made
   * available to give the calendar adapter access to useful information such as the UserInfo map,
   * session data, etc. These items can be used to identify the user, provide access to
  ...
  public CalendarEventSet getEvents(
      CalendarConfiguration calendar, Interval interval, PortletRequest request)
      throws CalendarException;

  /**
   * Get hyper link for the defined calendar. In cases where the calendar resource has a web
   * interface, this method allows provides access to the url.
   *
   * @param calendar calendar configuration for which to retrieve events
   * @param interval time period for which to retrieve events

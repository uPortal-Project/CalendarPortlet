 ...
 * <ol>
 *     <li>The configured Exchange Impersonation trusted service account credentials if the username and password
 *         are configured in portlet preferences</li>
 *     <li>The user's username (login id) and cached password if the Exchange NTLM domain is specified.
 *         Typically used for an on-premise Exchange server without using Exchange Impersonation.</li>
 *     <li>The user's email address and cached password if the Exchange NTLM domain is not specified.
 *         Typically used for Office365 integration without using Exchange Impersonation.</li>
 * </ol>
 * <br/>
 * To allow for multiple exchange destinations (on-premise Exchange Server and off-premise Office365) and
 * potentially different environments (different credentials for test environment) the credentials are
 * obtained from a combination of portlet preferences and bean configuration (property files).
 *
 * @author Jen Bourey, jbourey@unicon.net
 * @version $Revision$
 */
public class ExchangeCredentialsInitializationService implements IExchangeCredentialsInitializationService {
    private final Logger logger = LoggerFactory.getLogger(this.getClass());
    public final static String PREFS_NTDOMAIN = "exchangeNtlmDomain";
    public final static String PREFS_IMPERSONATION_USERNAME = "exchangeImpersonationUsername";
    public final static String PREFS_IMPERSONATION_PASSWORD = "exchangeImpersonationPassword";
     ...
     * Office365 integration when not using Exchange Impersonation.
    ...
    /**
     * Return the configured NTLM Domain if set.  May be null or empty string.  Portlet preference value overrides
     * bean configuration value to allow for multiple EWS integrations (on premise plus off-premise).
     *
    ...
    public boolean usesExchangeImpersonation(PortletRequest request) {
        ...
        if (StringUtils.isBlank(exchangeImpersonationUsername)
            ...
            logger.error("Both {} and {} must be set if using Exchange Web Services (EWS) Impersonation, or"
                    + " both must be blank if not using EWS Impersonation", PREFS_IMPERSONATION_USERNAME,
    ...
    public String getImpersonatedAccountId(PortletRequest request) {
        if (usesExchangeImpersonation(request)) {
            ...
            if (StringUtils.isBlank(username)) {
                throw new CalendarException("Null Username obtained from user attribute " + usernameAttribute
                        + ". It must be non-null when using Exchange Impersonation");
            }
            if (StringUtils.isBlank(domainName)) {
                throw new CalendarException("Domain name must be specified in properties file or portlet "
